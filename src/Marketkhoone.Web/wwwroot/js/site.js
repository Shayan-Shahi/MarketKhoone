...
function initializeSelect2() {
    //وارد مودال بشو و اگر کلاس کاستوم سلکت تو وجود داشت، فعالشون کن
...
//از این مثلا در صفحه ایجاد فروشنده برای سلکت توی استان ها استفاده میشه
...
if (jQuery.validator) {
    ...
    jQuery.validator.addMethod('isImage', function (value, element, param) {
        ...
        //کاربر در صقحه ایجاد محصول در تب بارگذاری تصاویر
        //آپلود رو میزنه و از کامپیوترش چند تا عکس انتخاب میکنه
        //بعد پشیمون میشه
        //دوباره دکمه آپلود رو میزنه و از کامپیوترش چند تا عکس حالا یا همون عکس ها رو انتخاب میکنه
        // الان اینپوت هایی با آیدی زیر ایجاد شدن 
        // که برای عکس های سری اول هم هستن باید اون اینپوت ها رو برداریم
...
function initializingAutocomplete() {
    ...
    //بر روی همه المنت هایی که کلاس اوتو کامپیلیت دارن
    //فور ایچ بزن
    $('.autocomplete').each(function () {

        //هر کدومشون ارتیبیوته
        //autocomplete-search-url
        //رو داشت
        var currentSearchUrl = $(this).attr('autocomplete-search-url');

        //آیدیش رو بگیر-نمشه از کلاس استفاده کرد، مجبوریم از آیدی استفاده بکنیم
...
$(document).on('click',
    '.get-html-with-ajax',
    function () {
        var functionNameToCallOnClick = $(this).attr('functionNameToCallOnClick');
        if (typeof window[functionNameToCallOnClick] === 'function') {
            window[functionNameToCallOnClick](this);
        }
...
$(document).on('submit', 'form.custom-ajax-form', function (e) {
    ...
    //میخواهیم در برخی از فرم هایی که این اتریبیوت رو دارن، فرم ها تا زمانی که ما کلوز نکردیم، بسته نشن
    //مثلا اضافه کردن فبچر
...
if (jQuery.validator) {
    ...
    //برای حالتی که در فرم
    // چک باکس داریم
    //مثل صفحه ی ثبت نام کاربر
...
//Url
//یو آر الی هست که   کاربر گرفته
//formData
//مقداری که از طریق مثلا Create.cshtml وارد میشه
//===  سه مساوی
// یعنی برابری مطلق
//برای مقدار یک متغیر نمیتوان ازش استفاده کرد فقط در مواردی که رشته ثابت میخواهیم چک کنیم استفاده میکنیم
...
// html
// از سمت سرور
//فانک پایینی تفاوتش با بالایی توی اینه که بالایی در آخر یک
//data.data
// برگشت میزنه، یک جیسون برگشت میزنه
// ما میخوایم
//html
//برگشت بزنیم
function getHtmlWithAJAX(url, formData, functionNameToCallInTheEnd, clickedButton, loading = true) {
    ...
    $.ajax({
        ...
        success: function (data) {
            //بله درسته، گفتیم جیسون برگشت  نمیزنیم، ولی برای حالتی که خطا رخ میده میگیم جیسون برگشت بزن ...خخخخ
            if (data.isSuccessful === false) {

                showToastr('warning', data.message);
            } else { 
                //این دیتای پایینی یک جیسون نیست، یک پارشل ویوو است
...
$('.multiple-images-preview-input').change(function () {
    ...
    //قبل اضافه کردن هر عکسی، یه بار کلا ویوو رو خالی کن 
...
$(function () {
    ...
    //اون تکست اریاهایی که این اتریبوت رو دارن
    ///بهشون بخش اپلود عکس رواضافه بکن
    ...
    //جلسه 107 رو ببین
    //این قسمت تو جسلسه 109 هستش
    // Initialize TinyMCE upload image plugin
    //روی تمامی تکست اریا هایی که کلاس
    //custom-tinymce
    //دارن فور ایچ بزن
    $('textarea.custom-tinymce').each(function () {
        var elementId = $(this).attr('id');
